...
class Pengajuan extends CI_Controller 
{
	...
	*/
	...
	var $mainPk="NIM";
	var $viewLink="Pengajuan";
	var $breadcrumbTitle="Pengajuan";
	...
	*/
	...
	public function index()
	{
		...
		foreach($renderTemp->result() as $row)
		{
			if($row->status==0)
			{
				$row->status="<a href='".base_url()."koordinator/Pengajuan/verifikasi/".$row->id."/1'><button class='btn btn-success btn-block'>Verifikasi</button></a>";
				$row->status.="<a href='".base_url()."koordinator/Pengajuan/verifikasi/".$row->id."/2'><button class='btn btn-danger btn-block'>Tolak</button></a>";
			}
			else
			if($row->status==1)
			{
				$row->status="<a href='#'><button class='btn btn-primary btn-block'>Diterima</button></a>";
			}
			else
			if($row->status==2)
			{
				$row->status="<a href='#'><button class='btn btn-warning btn-block'>Ditolak</button></a>";
			}
		}
		...
		//render view
		//$this->fn->getheader();
		$output['title'] = "Koordinator Panel System - Pengajuan Judul Tugas Akhir";
        $output['heading'] = "Pengajuan judul Tugas Akhir";
		$this->load->view("koordinator/daftar_judul/index",$output);
		//$this->fn->getfooter();
	}
